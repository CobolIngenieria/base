  ...
  function mfnBuilder() {
    ...
    builder.on('click', '.mfn-element-edit', function() {
      ...
      $('.mfn-field-color', popup).each(function() {
        var cp = $(this);
        var cp_input = $('input.has-colorpicker', cp).clone();
    ...
    /**
     * Element | Edit | Switch editor
     * Switch editor between Visual and HMTL in Visual Editor item
     */

    builder.on('click', '.mfn-switch-editor', function(e) {

      // default link action should not be taken

      e.preventDefault();

      // check current editor state and switch it
    ...
    builder.on('click', '#mfn-migrate .btn-import', function() {
      ...
      var el = $(this).siblings('#mfn-items-import');

      el.attr('name', el.attr('id'));

      if ($('#save-post').length || $('.editor-post-save-draft').length) {
        // save draft
        // WP 4.9 #save-post, WP 5.0 .editor-post-save-draft
        $('#save-post, .editor-post-save-draft').trigger('click');
      } else {
        // publish
        // WP 4.9 #publish, WP 5.0 .editor-post-publish-button
        $('#publish, .editor-post-publish-button').trigger('click');
      }
    ...
    builder.on('click', '#mfn-migrate .btn-template', function() {
      ...
      var el = $(this).siblings('#mfn-items-import-template');
     ...
     * TODO: works only with WP 4.9.8
     */

    $('#wp-content-wrap .wp-editor-tabs').prepend('<a class="wp-switch-editor switch-seo" id="content-seo">Builder &raquo; SEO</a>');

    $('body').on('click', '#content-seo', function() {
      ...
      if (confirm("This option is useful for plugins like Yoast SEO to analize post content when you use Muffin Builder.\nIt will collect content from Muffin Builder Elements and copy it into the WordPress Editor.\n\nCurrent Editor Content will be replaced.\nYou can hide the content if you turn \"Hide the content\" option ON.\n\nPlease remember to Publish/Update your post before & after use of this option.\nContinue?")) {
        ...
        var items_decoded = $('#mfn-items-seo-data').val();
        $('#content-html').trigger('click');
        $('#content').val(items_decoded).text(items_decoded);
